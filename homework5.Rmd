...
```{r}
#function that takes in a vector of followup.times and censors them so that 365 is the maximum, then returns the censored vector
...
```{r, tidy=TRUE}
#function that takes in whole dataframe and appens an indicator column for whether or not individuals were lost fo followup
calculate.losstofollowup <- function(dataframe)
{
  for (i in 1:nrow(dataframe))
  {
    ...
    {
      if (as.integer(difftime(dataframe$last.visit[i],dataframe$init.date[i],units="days")) <= 365) #if not dead, and they haven't had a followup after the first year, then they are "lost-to-followup"
      {
        dataframe$loss.to.followup[i] <- 1 
      }
      else dataframe$loss.to.followup[i] <- 0 #if not dead, and they have had a followup after the first year, then they werent "lost-to-followup"
    }
  }
  return(dataframe)
}
...
#find gaps between observation i and the observation below it that are greater than 365 days (this is where we will need to insert rows)
find.gaps <- function(id)
{
  ...
  {
    gaps[i] <- as.integer(difftime(id.vector[i+1,2],id.vector[i,2],units="days")) #days from visit 1 to visit 2, if >365 insert row below row i
  }
  return(gaps)
}
...
{
  id.vector <- subset.id(id)  
  a1c.mean <- mean(id.vector$a1c, na.rm=TRUE) #na.rm is important because of the missing values, so if you don't set this to true the means will be 0
...
get.firstname <- function(textRow)
{
    if (length(row.period.positions)>0)
    {
      ...
      {
        firstname[i] <- substr(text[i,],row.cap.positions[2],row.period.positions[1]) #include middle initial in firstname
      }
      else
      {
        firstname[i] <- substr(text[i,],row.cap.positions[2],(row.number.positions[1]-1))
      }
    }
    else
    {
...
replace.O <- function(textRow)
{
  if (substr(addr[41,],(nchar(addr[41,])-4),(nchar(addr[41,])-4)) == "-")
  {
    substr(textRow,(nchar(textRow)-9),nchar(textRow)) <- sub("O", "0",substr(textRow,(nchar(textRow)-9),nchar(textRow)))   
  }
  else
  {
    substr(textRow,(nchar(textRow)-4),nchar(textRow)) <- sub("O", "0",substr(textRow,(nchar(textRow)-4),nchar(textRow)))  
  }
  return(textRow)
}
...
fixText <- function(text)
{
  ...
  for (i in 1:nrow(text))
  {
    text[i,] <- replace.O(text[i,]) #replace incorrect Os in zipcodes with 0s 
    ...
    state[i] <- substr(text[i,],(row.cap.positions[length(row.cap.positions)-1]),row.cap.positions[length(row.cap.positions)])
    zip[i] <- substr(text[i,],(row.cap.positions[length(row.cap.positions)]+1),nchar(text[i,]))
    streetname[i] <- substr(text[i,],(find.first.jump(row.number.positions)+1),(row.cap.positions[length(row.cap.positions)-2]-1))
    city[i] <- substr(text[i,],(row.cap.positions[length(row.cap.positions)-2]),(row.cap.positions[length(row.cap.positions)-1]-1))  
  }
